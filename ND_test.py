##A function designed to record ground truth data for neural dust
##testing using the TDT RZ2 system. Includes functions to measure the 
##visual tuning properties of neurons by displaying drifting gratings.
...
SAVE_LOC = r"C:\Data\nd\6_17_16_ND_CF\board_1_4.hdf5"
#r"C:\Users\TDT-RAT\Desktop\test1.hdf5"
FIG_LOC = r"C:\Data\nd\6_17_16_ND_CF\board_1_4_"
...
def run_orientations(plot = False, savefigs = False):
	...
	for setN in range(NUM_SETS):
		...
		for repN in range(DIRECTIONS.size):
			...
			dset = set_group.create_dataset(str(DIRECTIONS[repN]), (num_samples,), dtype = 'f')
			...
			#raw trace
			trace_data = np.squeeze(RZ2.stream_data("raw_data", num_samples, 1, "F32", "float"))
			dset[:] = trace_data
			if plot:
				lfp = butter_bandpass_filter(trace_data, 0.5, 300, fs, 1)
				spike = butter_bandpass_filter(trace_data, 300, 5000, fs, 5)
				p_data.set_ydata(trace_data)
				z_data.set_ydata(trace_data)
				s_data.set_ydata(spike)
				l_data.set_ydata(lfp)
				ax1.set_ylim(trace_data.min(), trace_data.max())
				ax2.set_ylim(trace_data.min(), trace_data.max())
				ax3.set_ylim(spike.min(), spike.max())
				ax4.set_ylim(lfp.min(), lfp.max())
				fig1.suptitle(str(DIRECTIONS[repN])+" Degrees", fontsize = 18)
				fig1.canvas.draw()
				if savefigs:
					fig1.savefig(FIG_LOC+str(DIRECTIONS[repN])+".png", format = 'png')
